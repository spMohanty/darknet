...
void run_coco(int argc, char **argv)
{
    ...
    // loop over all known classes and load the label image if available
    printf("\nKnown categories:\n");
    ssize_t i_label_length;    
    int i_label_cnt;    
    
    for ( i_label_cnt = 0; i_label_cnt < i_num_cl; i_label_cnt++ )
    {
        
	//determine length of label
	i_label_length =  sizeof( coco_classes[i_label_cnt] );
	
        // and copy it to the global array
	c_class_names[i_label_cnt] = calloc(i_label_length, sizeof(char));
        memcpy(c_class_names[i_label_cnt], coco_classes[i_label_cnt] /* Offset */, i_label_length /* Length */);


        char c_buff[256];
        sprintf(c_buff, "data/labels/%s.png", c_class_names[i_label_cnt]);

	//try to access the label image for visualization
        if( access( c_buff, F_OK ) != -1 ) {
            printf("%s\n",c_class_names[i_label_cnt]);
            // file exists
            image cl_img = load_image_color(c_buff, 0, 0);
            img_class_labels[i_label_cnt] = cl_img;
        } else {
            // file doesn't exist
    ...
    if(0==strcmp(argv[2], "test"))
    {
        ...
        test_coco(c_cfg, c_weights, c_filename, thresh);
    }
    else if(0==strcmp(argv[2], "train")) 
    {
      train_coco(c_cfg, c_weights);
    }
    else if(0==strcmp(argv[2], "valid")) 
    {
      validate_coco(c_cfg, c_weights);
    }
